...
def pictor_decoder(features):
	with tf.name_scope("pictor_decode") as scope:
		# shape is 1, 8, 8, 8
		filters1 = 16
		w1 = weight_variable([3,3,filters1,8],name = "w1", )
		b1 = bias_variable([1], name = "b1", )
		...
		return w4, reconstruction


def pictor_autoencoder(x):
	features = pictor_encoder(x)
	_, reconstruction = pictor_decoder(features)
...
def pictor_reconstruct(x):
	features = pictor_encoder(x)
	sample, reconstruction = pictor_decoder(features)

	return features, sample, reconstruction

learning_rate = .1
pictor_error = pictor_autoencoder(x)
train_pictor = tf.train.GradientDescentOptimizer(learning_rate).minimize(pictor_error)
features, sample_weight, pictor_reconstruction = pictor_reconstruct(x)
...
with tf.Session() as sess:
	...
	for i in range(total_rounds):
		...
		if i % 1000 == 0:
			...
			feature, sample_, conv_img = sess.run([features,sample_weight, pictor_reconstruction], feed_dict={x: input_})
			conv_arr = np.array(conv_img.reshape(128,128))
